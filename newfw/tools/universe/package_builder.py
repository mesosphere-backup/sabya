...
class UniversePackageBuilder(object):
    ...
    def _get_newfw_mapping_for_content(self, orig_content):
        '''Returns a newfw mapping (dict) for the following cases:
        ...
        - Custom environment params like 'NEWFW_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default newfw values (may be overridden via eg NEWFW_PACKAGE_VERSION envvars):
        now = time.time()
        newfw_mapping = {
        ...
        # look for any 'sha256:filename' newfw params, and get shas for those.
        # this avoids calculating shas unless they're requested by the newfw.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            newfw_mapping['sha256:{}'.format(
                shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom NEWFW_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('NEWFW_'):
                # 'NEWFW_SOME_KEY' => 'some-key'
                newfw_mapping[env_key[len('NEWFW_'):].lower().replace(
                    '_', '-')] = env_val

        return newfw_mapping

    def _apply_templating_to_file(self, filename, orig_content):
        newfw_mapping = self._get_newfw_mapping_for_content(orig_content)
        new_content = orig_content
        for newfw_key, newfw_val in newfw_mapping.items():
            new_content = new_content.replace('{{%s}}' % newfw_key,
                                              newfw_val)
        ...
        newfw_keys = list(newfw_mapping.keys())
        newfw_keys.sort()
        for key in newfw_keys:
            logger.info('  {{%s}} => %s' % (key, newfw_mapping[key]))
