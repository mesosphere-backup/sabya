...
class UniversePackageBuilder(object):
    ...
    def _get_sabyafw_mapping_for_content(self, orig_content):
        '''Returns a sabyafw mapping (dict) for the following cases:
        ...
        - Custom environment params like 'SABYAFW_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default sabyafw values (may be overridden via eg SABYAFW_PACKAGE_VERSION envvars):
        now = time.time()
        sabyafw_mapping = {
        ...
        # look for any 'sha256:filename' sabyafw params, and get shas for those.
        # this avoids calculating shas unless they're requested by the sabyafw.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            sabyafw_mapping['sha256:{}'.format(
                shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom SABYAFW_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('SABYAFW_'):
                # 'SABYAFW_SOME_KEY' => 'some-key'
                sabyafw_mapping[env_key[len('SABYAFW_'):].lower().replace(
                    '_', '-')] = env_val

        return sabyafw_mapping

    def _apply_templating_to_file(self, filename, orig_content):
        sabyafw_mapping = self._get_sabyafw_mapping_for_content(orig_content)
        new_content = orig_content
        for sabyafw_key, sabyafw_val in sabyafw_mapping.items():
            new_content = new_content.replace('{{%s}}' % sabyafw_key,
                                              sabyafw_val)
        ...
        sabyafw_keys = list(sabyafw_mapping.keys())
        sabyafw_keys.sort()
        for key in sabyafw_keys:
            logger.info('  {{%s}} => %s' % (key, sabyafw_mapping[key]))
