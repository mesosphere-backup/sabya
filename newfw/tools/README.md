...
- **[publish_aws.py](#publish_awspy)**: Given a universe newfw and a set of build artifacts to be uploaded, creates a 'stub universe' package and uploads it along with the provided artifacts to a dev S3 bucket (ideally a directory with expiration configured to 7d or so).
- **[publish_http.py](#publish_httppy)**: Given a universe newfw and a set of build artifacts to be uploaded, creates a 'stub universe' package and hosts it from a local HTTP server along with any provided artifacts.
...
- **[universe_builder.py](#universe_builderpy)**: Underlying script which builds a `stub-universe.zip` given the necessary newfw data (package name, version, upload dir, ...). This is called by `publish_aws.py` and `publish_http.py` after autogenerating an upload directory.
...
- [Package newfw with placeholders](#package-newfw-with-placeholders)
- [Build script and artifacts](#build-script-and-artifacts)
- [Release script](#release-script)

### Package newfw with placeholders

The package newfw is the only particularly formal requirement, while the rest is fairly ad-hoc.
...
Once you have populated the package newfw directory, you should update it to include the following placeholders:
...
If you are providing binary CLI modules with your service, the tooling supports automatically populating the package newfw with any required `sha256sum` values.
...
You may include custom placeholders of the form `{{custom-placeholder}}` anywhere in your newfw. This can be useful for e.g. passing the path to a per-build docker tag, a randomly generated key, or anything else that you expect to change on a per-build basis.

These parameters can then be filled in by passing a `NEWFW_CUSTOM_PLACEHOLDER` environment variable with the desired value when calling `publish_aws.py` or `publish_http.py`. You can learn more about this feature in the documentation for `publish_aws.py` and `publish_http.py`, below.
...
```
$ ./publish_aws.py <package-name> <newfw-package-dir> [artifact files ...]
```
...
- `NEWFW_<SOME_PARAM>`: Inherited by `universe_builder.py`, see below.
...
```
$ ./publish_http.py <package-name> <newfw-package-dir> [artifact files ...]
```
...
The provided universe files may contain newfw parameters of the form `{{some-param}}`. The following parameters are filled by default:
...
In addition to these default newfw parameters, the caller may also provide environment variables of the form `NEWFW_SOME_PARAM` whose value will automatically be inserted into newfw fields named `{{some-param}}`. For example, running `NEWFW_DOCKER_IMAGE=mesosphere/docker-image ./universe_builder.py` would result in any `{{docker-image}}` parameters being replaced with `mesosphere/docker-image`.

A universe newfw is effectively a directory with the JSON files that you want to include in your Universe package, with newfw paramters provided in the above format. For some real-world examples of universe newfws, take a look at [Kafka's](https://github.com/mesosphere/dcos-kafka-service/tree/master/universe/) or [Cassandra's](https://github.com/mesosphere/dcos-cassandra-service/tree/master/universe/) newfws.
...
```
$ NEWFW_SOME_CUSTOM_STRING="this text replaces any instances of {{some-custom-string}}" \
./universe_builder.py \
    kafka \
    1.2.3-4.5.6 \
    dcos-kafka-service/universe/ \
...
As described above, any `NEWFW_<SOME_PARAM>` values will automatically be inserted into newfw slots named `{{some-param}}`. No other environment variables are needed.
